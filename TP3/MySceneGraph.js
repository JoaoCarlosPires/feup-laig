...
class MySceneGraph {
    ...
    constructor(filename, scene) {
        ...
        if (filename == "garage.xml") {
            scene.graph = this;
            scene.garageGraph = this;  
        } else if (filename == "room.xml") {
            scene.roomGraph = this;
        } 
    ...
    parseViews(viewsNode) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            this.scene.cameras[idCamera] = camera;
            this.scene.cameraList[numCameras] = idCamera;
            numCameras++;
        ...
        if (numCameras == 0) {
            ...
            this.scene.interface.setActiveCamera(this.scene.camera);
            this.scene.cameras["defaultCamera"] = defaultCamera;
            this.scene.cameraList[0] = "defaultCamera";
        } else {
            this.log("Parsed views and created cameras");
        }

        try {
            var folder = this.scene.interface.gui.addFolder('Camera');
            folder.open();
            folder.add(this.scene, 'selectedCamera', this.scene.cameraList).onChange(this.scene.onSelectedCameraChanged.bind(this.scene)).name('Camera');
        } catch (e) {
        }
   ...
   */
                    ...
                    case 'leaf':
                        ...
                        switch (leaf_type) {
                            ...
                            case 'defbarrel':
                                ...
                                break;
                            case 'objfile':
                                var file = this.reader.getString(grandgrandChildren[l], 'file');
                                var primitive = new CGFOBJModel(this.scene, file);
                                leaf.push(primitive);
                                break;
                            case 'gamepiece':
                                var id = this.reader.getFloat(grandgrandChildren[l], 'id');
                                var primitive = new GamePiece(this.scene, id, textureID);
                                pieces.push(primitive);
                                break;
                            case 'auxiliarpiece':
                                var primitive = new AuxiliarPiece(this.scene);
                                leaf.push(primitive);
                                break;
                            case 'gameboard':
                                var x1 = this.reader.getFloat(grandgrandChildren[l], 'x1');
                                ...
                                var x2 = this.reader.getFloat(grandgrandChildren[l], 'x2');
                                var y2 = this.reader.getFloat(grandgrandChildren[l], 'y2');
                                var primitive = new MyGameBoard(this.scene, x1, y1, x2, y2, afs, aft);
                                leaf.push(primitive);
                                break;
                            case 'passbutton':
                                var player = this.reader.getFloat(grandgrandChildren[l], 'player');
                                var primitive = new PassButton(this.scene, player);
                                leaf.push(primitive);
                                this.scene.passButtons.push(primitive);
                                break;
                            default:
    ...
    displayLoop(id, parentMaterial, parentTexture) {
        ...
        for (var k = 0; k < this.nodes[id][4].length; k++) {
            try {
                this.nodes[id][4][k].check();
                this.material = new CGFappearance(this);
                this.material.setAmbient(0, 1, 1, 1);
                this.material.setDiffuse(1, 0, 1, 1);
        ...
        //Pieces
        for (var k = 0; k < this.nodes[id][6].length; k++) {
            this.nodes[id][6][k].display();
        }
